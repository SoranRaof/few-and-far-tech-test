...
class Report {
  ...
  getDonationsExports = async () => {
    ...
    console.log(
      `retrieving donations export from API. attempt No.: ${this.getReportAttempts}`
    );
  ...
  getAllDonationsBySupporterId = async (supporterId) => {
    if (!this.donationsExports) {
      ...
      const allSupporterDonos = this.donationsExports
        .filter((x) => x.supporter_id === supporterId)
        .map((x) => (x = x.amount));
  ...
  getLifetimeReportOnAllSupporters = async () => {
    if (!this.supporters) {
      ...
      for (const supporter of this.supporters) {
        const lifetimeSupport = await this.getLifetimeValueOfSupporterById(
          supporter.id
        );
...
// Call getLifetimeReportOnSupporterByName for a life time report on a specific supporter.
// Please comment out lifetimeReportAll on line 20.
